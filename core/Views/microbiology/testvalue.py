...
def micro_biology_testvalue(request):
    try:
        ...
        # bound the payload as micro biology test values grow; update any
...
def mb_save_test_value(request):
    logger.debug(f"DEBUG: Request received: Method={request.method}, User={request.user}, Data={request.data}")

    payload = request.data
    employee_id = payload.get('auth-user-id')
    logger.debug(f"DEBUG: POST payload: {payload}")
    ...
    try:
        ...
        logger.warning("DEBUG: Patient not found")
...
def mb_get_test_values(request):
    ...
    # NOTE: response shape changed from a bare JSON array to a paginated
    # object ({"data": [...], total_count, total_pages, current_page}) to
    # bound the payload as MBTestValue grows; update any frontend caller
    # that expected a raw array here.
    total_count = len(patient_data)
    page_obj, page_meta = paginate_queryset(patient_data, request)
    return JsonResponse({
        "data": list(page_obj),
