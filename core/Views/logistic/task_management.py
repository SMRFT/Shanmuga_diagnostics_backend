...
def create_logistics(request):
    try:
        if request.method == 'GET':
            ...
            serializer = LogisticsSerializer(page_obj, many=True)
            return Response(
                {'count': page_meta['total_count'], 'results': serializer.data, **page_meta},
...
def logistics_tat_report(request):
    ...
    try:
        ...
        for task in logistics_tasks:
            ...
            if task.sampleordertime:
                try:
                    ...
                    logger.error(f"Error parsing sampleordertime: {e}")
            ...
            if task.sampleacceptedtime:
                try:
                    ...
                    logger.error(f"Error parsing sampleacceptedtime: {e}")
            ...
            if task.samplepickeduptime:
                try:
                    ...
                    logger.error(f"Error parsing samplepickeduptime: {e}")
            ...
            if order_time and accepted_time:
                try:
                    ...
                    logger.error(f"Error calculating order_to_accept_tat: {e}")
            ...
            if accepted_time and picked_up_time:
                try:
                    ...
                    logger.error(f"Error calculating accept_to_pickup_tat: {e}")
            ...
            if order_time and picked_up_time:
                try:
                    ...
                    logger.error(f"Error calculating total_tat: {e}")
        ...
        # Paginate only the detail rows; 'summary' above is computed over the
        # full matching set so totals/averages stay correct regardless of page.
        page_obj, page_meta = paginate_queryset(tat_data, request)
        response_data['data'] = list(page_obj)
        response_data.update(page_meta)
