...
def compare_test_details(request):   
    ...
    for test_item in test_list:
        ...
        if sample_status == "Received" or sample_status == 'Unknown':
            # First try to get test details using test_name
            test_details_cursor = core_testdetails_collection.find({"test_name": test_name})
            
            # If no results by test_name and we have test_id, try by test_id
            ...
            for test_detail in test_details_list:
                ...
                # For parameterized tests - FIXED DEVICE SELECTION LOGIC
                ...
                # Group records by device to see what devices have data
                devices_with_data = {}
                for record in all_barcode_records:
                    device = record.get('DeviceID')
                    test_code = record.get('TestCode')
                    if device and test_code:
                        if device not in devices_with_data:
                            devices_with_data[device] = []
                        devices_with_data[device].append(test_code)
                
                print(f"DEBUG: Devices with data for barcode {barcode}: {devices_with_data}")
                
                # If device_id is specified in request and has data, prioritize it
                if device_id and device_id in parameters and device_id in devices_with_data:
                    selected_device = device_id
                    has_interface_data = True
                    print(f"DEBUG: Using specified device {device_id}")
                
                # If no specific device_id or it has no data, find device with most matching test codes
                if not has_interface_data:
                    best_match_device = None
                    best_match_count = 0
                    
                    for dev_id in devices_with_data:
                        if dev_id in parameters:
                            # Get test codes for this device from parameters
                            param_test_codes = [param.get('test_code') for param in parameters[dev_id] if param.get('test_code')]
                            # Get test codes for this device from interface data
                            interface_test_codes = devices_with_data[dev_id]
                            
                            # Count how many test codes match
                            matches = len(set(param_test_codes) & set(interface_test_codes))
                            
                            print(f"DEBUG: Device {dev_id} - Param codes: {param_test_codes[:3]}..., Interface codes: {interface_test_codes}, Matches: {matches}")
                            
                            if matches > best_match_count:
                                best_match_count = matches
                                best_match_device = dev_id
                    
                    if best_match_device:
                        selected_device = best_match_device
                        has_interface_data = True
                        print(f"DEBUG: Selected device {best_match_device} with {best_match_count} matches")
                
                # If still no match found, check if any device matches by DeviceID regardless of test codes
                if not has_interface_data:
                    for device in devices_with_data:
                        if device in parameters:
                            selected_device = device
                            has_interface_data = True
                            print(f"DEBUG: Using device {device} based on DeviceID match")
                            break
                
                # If no data in interface_testvalue at all, show first device parameters
                if not has_interface_data:
                    selected_device = sorted(parameters.keys())[0]  # Get first device
                    print(f"DEBUG: No data found, using default device {selected_device}")
                ...
                if selected_device and selected_device in parameters:
                    ...
                    print(f"Processing parameters for device: {selected_device}")
                    ...
                    for param in param_list:
                        ...
                        if has_interface_data:
                            # Get test value from interface_testvalue collection with STRICT Pending filter
                            test_value_query = {
                                "Barcode": barcode,
                                "TestCode": test_code,
                            ...
                            # If we found a Pending record, track it for processing
                            if test_value_doc:
                                processed_records.append({
                                    'barcode': barcode,
                                    'test_code': test_code,
                                ...
                                test_value = test_value_doc.get('Value', '')
                                processing_status = test_value_doc.get('processingstatus', 'N/A')
                                lab_unique_id = test_value_doc.get('lab_unique_id', 'N/A')
                                created_date = test_value_doc.get('CreatedDate')
                                received_date = test_value_doc.get('Receiveddate')
                            else:
                                # No Pending record found for this parameter, show empty values
                                test_value = ''
                            ...
                            # No data in interface_testvalue, show empty values
                            test_value = ''
                            processing_status = 'No Data'
                            lab_unique_id = 'N/A'
                            created_date = None
